...
p1 = ggplot(data = tmp, mapping = aes(x = fitness, y = log10(X))) + theme_classic() + theme(legend.position = 'bottom', legend.direction = 'vertical') +
  ...
  labs(tag = 'A', linetype = 'γ', color = 'δ') + ylab('ToE log10(h)') + xlab(expression(paste('fitness (',omega,')', sep = ''))) + theme(legend.title.align=0.5) + guides(linetype = guide_legend(override.aes = c(colour = 'black')))
...
p2 = ggplot(data = tmp, mapping = aes(x = log10(conjugation), y = log10(X))) + theme_classic() + theme(legend.position = 'bottom', legend.direction = 'vertical') +
  ...
  labs(tag = 'B', linetype = 'δ', color = 'ω') + ylab('ToE log10(h)') + xlab(expression(paste('conjugation rate (',gamma,')', sep = ''))) + theme(legend.title.align=0.5) + guides(linetype = guide_legend(override.aes = c(colour = 'black')))
...
p3 = ggplot(data = tmp, mapping = aes(x = log10(loss), y = log10(X))) + theme_classic() + theme(legend.position = 'bottom', legend.direction = 'vertical') +
  ...
  labs(tag = 'C', linetype = 'γ', color = 'ω') + ylab('ToE log10(h)') + xlab(expression(paste('loss rate (',delta,')',sep = ''))) + theme(legend.title.align=0.5) + guides(linetype = guide_legend(override.aes = c(colour = 'black')))
...
colnames(tmp) = c('interaction value',colnames(tmp)[2:3],'Dunn test*'); x1 = tmp; x1$`interaction value` = as.character(x1$`interaction value`);  x1 = x1[order(x1$`interaction value`),]
...
for(i in x6$Group) { x4[i, 'Fisher exact test pairwise Nominal Independence*'] = x6[x6$Group == i, 'Letter']}
...
p1 = ggplot(data = loss_ex[loss_ex$lossxy != '1',], mapping = aes(x = log10(no_interaction), y = log10(X_XY), color = lossxy)) + 
  theme_classic() + labs(tag = 'A', color = expression(delta)) + geom_abline(slope = 1, intercept = 0) + geom_point(shape = 21, fill = NA, alpha = 3/4) +  
  xlab(bquote(atop(delta==0,'ToE log10(h)'))) + ylab(bquote(atop(delta!=0,'ToE log10(h)'))) + theme(legend.title.align = 0.5) + scale_color_manual(values = my_palete[2], name = bquote(atop('effect on loss',paste('(',delta,')'))))
p1
```
...
p2 = ggplot(data = loss_ex, mapping = aes(x = log10(alone), y = log10(X_XY), color = lossxy)) + 
  theme_classic() + labs(tag = 'B', color = expression(delta)) + geom_abline(slope = 1, intercept = 0) + geom_point(shape = 21, fill = NA, alpha = 3/4) +  
  xlab('Y absent\nToE log10(hours)') + ylab('Y present\nToE log10(hours)') + theme(legend.title.align = 0.5) + scale_color_manual(values = my_palete[c(1:2)], name = bquote(atop('effect on loss',paste('(',delta,')'))))
p2
```
...
p2 = ggplot(data = tmp[tmp$variable != 'epistasis',], mapping = aes(x = value, y = ratio_alone, color = out_alone)) + 
  theme_classic() + labs(tag = 'B') + geom_point(position = 'jitter', shape = 21, fill = NA, alpha = 3/4) + facet_grid(.~var, scales = 'free', labeller = label_parsed) + theme(axis.text.x = element_text(angle = 30)) + 
